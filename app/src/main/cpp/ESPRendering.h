...
void DrawLine(ImVec2 start, ImVec2 end, ImVec4 color, int thickness, ImDrawList* background) {
    if(background) {
...
void DrawBox(ImVec4 rect, ImVec4 color, int thickness, ImDrawList* background) {
    ...
    DrawLine(v1, v2, color, thickness, background);
    DrawLine(v2, v3, color, thickness, background);
    DrawLine(v3, v4, color, thickness, background);
    DrawLine(v4, v1, color, thickness, background);
}
...
void DrawOutlinedBox2(float x, float y, float width, float height, ImVec4 color, int thickness, ImDrawList* background)
{
    DrawBox2(x-thickness+1, y-thickness+1, width + thickness*2 -2, height + thickness*2 -2, ImVec4(0,0,0,color.w), 1, background);
    DrawBox2(x, y, width, height, color, thickness, background);
    DrawBox2(x + thickness -1, y + thickness -1, width - thickness*2 + 2, height - thickness*2 +2, ImVec4(0,0,0,color.w), 1, background);
}
...
void DrawBones(void* character, int bone1, int bone2, ImDrawList* background){
    if(character != nullptr){
        Vector3 bone1Pos = helpers::getBonePosition(character, bone1);
        Vector3 bone2Pos = helpers::getBonePosition(character, bone2);
        Vector3 wsbone1 = helpers::WorldToScreenPoint(pointers::Camera$$get_main(), bone1Pos);
        Vector3 wsbone2 = helpers::WorldToScreenPoint(pointers::Camera$$get_main(), bone2Pos);
        wsbone1.Y = glHeight - wsbone1.Y;
        wsbone2.Y = glHeight - wsbone2.Y;
        if(wsbone1.Z > 0 && wsbone2.Z > 0){
            DrawLine(ImVec2(wsbone1.X, wsbone1.Y), ImVec2(wsbone2.X, wsbone2.Y), ImColor(255, 255, 255, 255), 3, background);
        }
    }
}

void DrawText(ImVec2 position, ImVec4 color, std::string text, ImFont* font, ImDrawList* background) {
    if(background) {
...
void CoolCrosshair(ImVec2 position, ImVec4 color, ImDrawList* background) {
